 ...
 */
uint8_t si4455_configuration_init( uint8_t* pSetPropCmd)
{
	uint8_t col,response,numOfBytes;
...
void si4455_part_info(void)
{
    ...
    Si4455Cmd.PART_INFO.PART    = (radioCmd[1]<<8)+radioCmd[2];
  
    Si4455Cmd.PART_INFO.PBUILD          = radioCmd[3];
    Si4455Cmd.PART_INFO.ID      = (radioCmd[4]<<8)+radioCmd[5];
...
void si4455_func_info(void)
{
    ...
    Si4455Cmd.FUNC_INFO.PATCH.uint8_t[MSB]   = radioCmd[3];
    Si4455Cmd.FUNC_INFO.PATCH.uint8_t[LSB]   = radioCmd[4];
    Si4455Cmd.FUNC_INFO.FUNC            = radioCmd[5];
    Si4455Cmd.FUNC_INFO.SVNFLAGS        = radioCmd[6];
    Si4455Cmd.FUNC_INFO.SVNREV.uint8_t[b3]   = radioCmd[7];
    Si4455Cmd.FUNC_INFO.SVNREV.uint8_t[b2]   = radioCmd[8];
    Si4455Cmd.FUNC_INFO.SVNREV.uint8_t[b1]   = radioCmd[9];
    Si4455Cmd.FUNC_INFO.SVNREV.uint8_t[b0]   = radioCmd[10];
}
...
void si4455_get_adc_reading(uint8_t ADC_EN, uint8_t ADC_CFG)
{
    ...
    Si4455Cmd.GET_ADC_READING.GPIO_ADC.uint8_t[MSB]      = radioCmd[0];
    Si4455Cmd.GET_ADC_READING.GPIO_ADC.uint8_t[LSB]      = radioCmd[1];
    Si4455Cmd.GET_ADC_READING.BATTERY_ADC.uint8_t[MSB]   = radioCmd[2];
    Si4455Cmd.GET_ADC_READING.BATTERY_ADC.uint8_t[LSB]   = radioCmd[3];
    Si4455Cmd.GET_ADC_READING.TEMP_ADC.uint8_t[MSB]      = radioCmd[4];
    Si4455Cmd.GET_ADC_READING.TEMP_ADC.uint8_t[LSB]      = radioCmd[5];
...
void si4455_get_modem_status( uint8_t MODEM_CLR_PEND )
{
    ...
    Si4455Cmd.GET_MODEM_STATUS.AFC_FREQ_OFFSET.uint8_t[MSB]  = radioCmd[6];
    Si4455Cmd.GET_MODEM_STATUS.AFC_FREQ_OFFSET.uint8_t[LSB]  = radioCmd[7];
}
